...
helm upgrade --install "$RELEASE" . -n "$NAMESPACE" --create-namespace -f values.yaml -f values-local.yaml -f "$SECRET" --set initJob.enabled=false --timeout 10m
...
helm upgrade "$RELEASE" . -n "$NAMESPACE" -f values.yaml -f values-local.yaml -f "$SECRET" --timeout 10m
